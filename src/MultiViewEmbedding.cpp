...
std::vector<double> MultiViewEmbedding(
  ...
  std::sort(indices.begin(), indices.end(), [&](size_t a, size_t b) {
    ...
    if (ma[0] != mb[0]) return ma[0] > mb[0];  // Higher rho first
...
std::vector<double> MultiViewEmbedding(
  ...
  // Parallel evaluation across feature stacks
  RcppThread::parallelFor(0, num_groups, [&](size_t g) {
    const auto& stack = vectors[g];

    // Compute performance metrics for this stack using SimplexBehavior
    auto metrics = SimplexBehavior(stack, target, lib_indices, pred_indices,
                                   num_neighbors, dist_metric, dist_average);

    pred_metrics[g] = metrics;
  }, threads_sizet);

  // Rank stacks by performance (ρ descending, RMSE ascending, MAE ascending)
  std::vector<size_t> indices(num_groups);
  std::iota(indices.begin(), indices.end(), 0);
  ...
  // Select top-performing stacks
  const size_t num_selected = std::min(static_cast<size_t>(top_num), num_groups);
  std::vector<size_t> selected(indices.begin(), indices.begin() + num_selected);

  // Construct a new 3D tensor of selected embeddings
  // Structure: [selected_stack][sample][dimension]
  std::vector<std::vector<std::vector<double>>> selected_embeddings;
  selected_embeddings.reserve(num_selected);

  for (auto g : selected) {
    selected_embeddings.push_back(vectors[g]);
  }

  // Predict using the selected embedding stacks
  // SimplexProjectionPrediction is assumed to support 3D input
  std::vector<double> result =
    SimplexProjectionPrediction(selected_embeddings, target, lib_indices, pred_indices, num_neighbors);
